			...
			<div class="content">
				...
				<div class="list-content">
					...
					<div class="block">
						<!--修饰块元素名称-->
						<div class="banner">
							<p class="tab fixed">帖子编辑<span class="hint">Article Editors</span></p>
						</div>
						<!--正文-->
						<div class="main">
							<!--表单-->
							<form id="form" action="" method="">
								...
								<div class="unit">
									...
									<div class="right" style="width: 60%">
										<input type="text" class="text" name="title" data-type="必填" placeholder="请输入帖子标题" value="${article.title}" />
									</div>
									<!--清浮动-->
									...
									<div class="right" style="width: 60%">
										<select id="type_id" name="typeId">
											<c:forEach items="${typeList}" var="typeInfo" varStatus="status" >
												<option value="${typeInfo.id}" <c:if test="${article.typeId==typeInfo.id}">selected</c:if>>${typeInfo.name}</option>
											</c:forEach>
										</select>
									</div>
									<!--清浮动-->
		...
		$.ajax({
			...
			success : function(rtn) {
				if (rtn.code=="000000") {
					...
					setTimeout(function() {
						// 跳转页面
						window.location.href = "${pageContext.request.contextPath}/admin/list.action";
					}, 2000);
				} else {
					javaex.optTip({
						content : rtn.message,
