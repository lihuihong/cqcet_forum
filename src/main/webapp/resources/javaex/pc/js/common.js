		...
		for (var i in classArr) {
			if (classArr[i].indexOf("grid-")>=0) {
				...
				$(this).children("div").each(function(k) {
					// 第一个div
					if (k==0) {
						$(this).css("margin-right", parseInt(spacing)/2 + "px");
					} else if (k==(length-1)) {
						// 最后一个div
						$(this).css("margin-left", parseInt(spacing)/2 + "px");
					} else {
						// 中间的div
						$(this).css({
							"margin-left" : parseInt(spacing)/2 + "px",
							"margin-right" : parseInt(spacing)/2 + "px"
						});
					}
				});
			}
